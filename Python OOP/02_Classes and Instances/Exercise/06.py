...
class PizzaDelivery:
    ...
    def pizza_ordered(self) -> str:
        ...
        self.ordered = True
        ingredients_for_print = [f"{key}: {value}" for key, value in self.ingredients.items()]
        return (
            f"You've ordered pizza {self.name} "
            f"prepared with {', '.join(ingredients_for_print)} and the price will be {self.price}lv."
        )
